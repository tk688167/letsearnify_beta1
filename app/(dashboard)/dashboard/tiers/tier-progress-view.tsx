...
// Professional Color Map — Full Dark Mode Support
const TIER_STYLES: Record<string, { badge: string, border: string, text: string, icon: string, bg: string, ring: string, accent: string }> = {
    ...
    BRONZE: {
        badge: "bg-orange-50 dark:bg-orange-900/40 text-orange-800 dark:text-orange-300",
    ...
    GOLD: {
        badge: "bg-yellow-50 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-300",
    ...
    PLATINUM: {
        badge: "bg-slate-50 dark:bg-slate-800/80 text-slate-800 dark:text-slate-300",
    ...
    DIAMOND: {
        badge: "bg-blue-50 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300",
    ...
    EMERALD: {
        badge: "bg-emerald-50 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300",
    ...
    <div className="space-y-8 animate-in fade-in duration-700 max-w-5xl mx-auto">
      ...
      <div className="relative overflow-hidden rounded-2xl text-white"
        ...
        <div className="relative z-10 px-5 sm:px-8 py-5 sm:py-6 text-center">
          ...
          <p className="text-amber-200/70 text-sm max-w-sm mx-auto mb-4">
            Unlock higher commissions as you progress through the tiers
          </p>
      ...
      <div className="relative pl-6 md:pl-0">
          ...
          <div className="space-y-6 sm:space-y-8">
             {TIERS.map((tierName, index) => {
                ...
                // Use minimum — a tier requires ALL criteria to be met, not just one.
                // Averaging misleads users into thinking they're making headway when
                // a critical requirement like team size is still at 0.
                   ...
                   >
                      ...
                      <div className={`group rounded-2xl border transition-all duration-300 overflow-hidden ${
                          ...
                          <div className={`px-4 sm:px-6 md:px-8 py-4 sm:py-5 flex flex-col sm:flex-row justify-between gap-3 sm:gap-4 sm:items-center border-b ${
                            isCurrent
                              ? "border-indigo-100/60 dark:border-indigo-800/40"
                              : "border-gray-100/60 dark:border-gray-700/40"
                          }`}>
                               <div className="flex items-center gap-4">
                                   ...
                                   <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-sm border flex-shrink-0 transition-all duration-300 ${
                                       isLocked
                                         ? "bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 grayscale opacity-50"
                                         : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700"
                                   }`}>
                                       {style.icon}
                                   </div>
                                   <div className="min-w-0">
                                       {/* Tier Name + Status Badge */}
                                       <h3 className={`text-base sm:text-lg font-bold flex items-center gap-2 flex-wrap ${
                                         isCurrent
                                           ...
                                           {isCurrent && (
                                             <span className="text-[9px] bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 rounded-full uppercase font-bold tracking-wide border border-indigo-200/60 dark:border-indigo-700/40">
                                               Active
                                             </span>
                                           )}
                               ...
                               {isCurrent ? (
                                   <div className="sm:text-right flex flex-col items-center sm:items-end gap-1">
                                    ...
                                    <div className="text-[10px] font-bold text-gray-400 dark:text-gray-600 uppercase tracking-widest sm:text-right">
                                      Locked
                                    </div>
                               ) : (
                                    <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-widest sm:text-right">
                                      Completed
                                    </div>
                               )}
                          </div>

                          {/* Card Body — Requirements (only show if not completed and not final) */}
                          {!isCompleted && !isFinal && (
                            <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-6 grid sm:grid-cols-2 gap-4 sm:gap-6">
                                
                                {/* ARN Points Requirement */}
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className={`text-xs font-bold ${isCurrent ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'}`}>
                                            {Math.round(user.arnBalance || (user.balance * 10)).toLocaleString()}
                                            <span className="text-gray-400 dark:text-gray-600 font-normal"> / {targetArn.toLocaleString()}</span>
                                        </span>
                                    </div>
                                    <div className="h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                        <div 
                                          className={`h-full rounded-full transition-all duration-1000 ${isCurrent ? 'bg-indigo-500 dark:bg-indigo-400' : 'bg-gray-300 dark:bg-gray-600'}`} 
                                ...
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className={`text-xs font-bold ${isCurrent ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'}`}>
                                            {currentDirectsInStep.toLocaleString()}
                                            <span className="text-gray-400 dark:text-gray-600 font-normal"> / {targetDirects.toLocaleString()}</span>
                                        </span>
                                    </div>
                                    <div className="h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                        <div 
                                          className={`h-full rounded-full transition-all duration-1000 ${isCurrent ? 'bg-purple-500 dark:bg-purple-400' : 'bg-gray-300 dark:bg-gray-600'}`} 
                          ...
                          {isFinal && (
                             <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-5">
                                 <div className={`p-3 sm:p-4 rounded-xl border flex items-center justify-center gap-2.5 text-sm font-medium ${
                                     isCurrent 
                                        ? "bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/50 dark:to-purple-950/50 border-indigo-100 dark:border-indigo-800/40 text-indigo-800 dark:text-indigo-300"
                                        : isCompleted
                                          ? "bg-emerald-50/50 dark:bg-emerald-950/30 border-emerald-200/60 dark:border-emerald-800/40 text-emerald-700 dark:text-emerald-400"
                                    ...
                                    <span className="text-xs sm:text-sm text-center">
                                        {isCurrent 
                                            ...
                                            : isCompleted
                                              ? "Maximum tier achieved. You are at the pinnacle!"
