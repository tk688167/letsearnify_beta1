...
// Helper to check tier eligibility
    const isTierEligible = (poolKey: string, tier: string | undefined): boolean => {
        ...
        return true; // Others open to all active members? (CBSP requires active member which is checked by isLocked prop usually)
                     // Actually CBSP logic is "5% of deposit". Distribution requires active member.
                     // Access to View? Open.
    }
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            {poolConfig.map((pool) => {
                ...
                // Locked if global lock is on OR (specific pool rule AND not eligible)
                // BUT "isLocked" prop usually means "User is not active member".
                // If User IS active member (isLocked=false), but NOT Gold (Royalty), we should show specific lock.
                
                const showLock = isLocked || !isPoolEligible;
                const lockReason = isLocked ? "Account Limited" : "Tier Restricted";
                    ...
                    <div className="relative z-10 p-6 flex flex-col h-full">
                        ...
                        <div className="mb-6">
                            ...
                            <div className={`text-2xl md:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r ${pool.accent}`}>
                                {showLock && !isLocked ? pool.data.formatted : pool.data.formatted} 
                                {/* Always show amount? Maybe blur if locked? Let's show it as motivation. */}
                            </div>
                        </div>
                        ...
                        {showLock && (
                             <div className="mt-4 pt-3 border-t border-gray-100/50">
                                <div className="flex items-center gap-2 justify-center text-xs font-bold text-gray-400 bg-gray-50 py-2 rounded-lg">
                                    ...
                                    {isLocked ? "Account Limited" : "Requires Upgrade"}
                                </div>
                             </div>
                        )}
