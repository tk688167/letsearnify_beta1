...
import { SpinReward, getStandardSegments } from "@/lib/spin-config"
...
export async function getSpinRewards(type: "FREE" | "PREMIUM"): Promise<SpinReward[]> {
    ...
    return getStandardSegments(rewards, type) as SpinReward[];
}
...
export async function upsertSpinReward(data: {
    ...
    const isNew = !data.id || data.id.startsWith("default-")
