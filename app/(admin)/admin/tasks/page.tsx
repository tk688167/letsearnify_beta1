            ...
            <tbody className="divide-y divide-gray-100">
               {tasks.map((task) => (
                   <tr key={task.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 font-mono text-green-600 font-bold">{task.reward.toFixed(0)} ARN</td>
            ...
            <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
                ...
                <form action={async (formData) => {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                         ...
                         <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Reward (ARN)</label>
                            <input name="reward" type="number" step="1" defaultValue={task?.reward} required className="w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:border-indigo-500"/>
                         </div>
                    </div>
